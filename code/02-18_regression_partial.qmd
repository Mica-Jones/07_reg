...
```{r}
#| message: false
reg_dfw <- read.csv(file = "../data/reg.csv") %>%
  clean_names() %>%
...
# boxplots do not work well for regression; they mainly only work for ANOVA. They can be used for EDA but shouldnt be included in publication or analysis
```
...
ggplot(reg_dfw, 
       aes(x = sr_ksha, 
           y = yield_mgha)) +
  geom_point() +
  geom_smooth() # geom_smooth gives a nice trendline and tentative error region 
...
```{r mod1 Standardized Residuals vs. Fitted, warning=FALSE}
ggplot(mod1_int_aug, 
       aes(x = .fitted, 
           y = .stdresid))+
...
```{r nd, warning=FALSE}
nd <- data_frame(sr_ksha = seq(40, 120, 1))
...
mod4_linp_randeff <- random.effects(mod4_linp) %>% 
  as.data.frame()
